...
import MenuDetailKasir from "../../../components/MenuDetailKasir";
import MenuDetailAdmin from "../../../components/MenuDetailAdmin";
import { useRouter } from "next/navigation";


const MenuDetailPage = ({ params }) => {
  ...
  const menuId = React.use(params).id;
  ...
  useEffect(() => {
    ...
    const fetchMenuDetails = async () => {
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL_BE}/api/menus/${menuId}`, {
          headers: {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-amber-800">Loading menu details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-xl border border-amber-200">
          ...
          <p className="text-amber-800">You don't have permission to view this menu.</p>
        </div>
      </div>
    );
  }
